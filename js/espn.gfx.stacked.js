...
d3.chart('stacked', {
	...
	initialize: function(options) {
		...
		leftWrapper = this.base.append('g').attr('class', chart.config.leftStackClass);

		rightWrapper = this.base.append('g').attr('class', chart.config.rightStackClass);
		...
		this.layer('rightAxis', rightWrapper, {
			dataBind: dataBind,
			insert: insert
		});
		...
		function onEnter() {
			...
			this.attr('y', function(d) {
				...
				.attr('x', function(d) {
					return (chart.x(d.y0) / 2 + chart.width() / 2) + space / 2;
				})
		...
		this.layer('leftAxis', leftWrapper, {
		...
		function onEnterLeft() {
			this.attr('y', function(d) {
				...
				.attr('x', function(d) {
					return (chart.width() - chart.x(d.y) - chart.x(d.y0)) / 2 - space / 2;
				})
				.attr('fill', function(d, i) {
					if (varyColors) {
	...
	transform: function(dataSrc) {
		...
		return {
			left: d3.merge(leftStack),
			right: d3.merge(rightStack)
		};
	}
